...
import employee.PayscalePromoter;
...
/**
 * The csv.CSVWriter class provides methods to write data to various CSV files including employees' pay scales, payslips, and part-time employee work hours.
 * @author Iwan Teague / Simon Alexander / Saif Khawaldeh
 */
public class CSVWriter {
    /**
     * Writes the incremented pay scale data to Employees.csv.
     * This method fetches pay scale data from the employee.PayscalePromoter and writes it to the CSV file.
     */
    public static void writeToCSVPayScale() {
        ...
        try (FileWriter writer = new FileWriter("csv/csv files/Employees.csv")) {
     ...
     * This method fetches payslip data from the payroll.PayslipGenerator and appends it to the Payslips.csv file.
     */
    public void writeCsvPaySlipGen() {
        PayslipGenerator paySlipGen = new PayslipGenerator();

        List<String[]> data = paySlipGen.createPayslips();
        try (FileWriter writer = new FileWriter("csv/csv files/Payslips.csv", true)) {
            for (int i = 0; i < data.size(); i++) {
                String[] row = data.get(i);
                writer.append(String.join(",", row));
                writer.append("\n");
    ...
    public static void writeCSVPartTime(String ppsNo, double hours, String date, double earned) {
        ...
        String filePath = "csv/csv files/PartTime.csv";
