...
import employee.EmployeeSalary;

import java.io.*;
import java.util.*;

/**
 * The payroll.PartTimeHourCalculator class is responsible for calculating the earned amount for part-time employees
...
public class PartTimeHourCalculator {
    ...
    public static void main(String[] args) {
        ...
        //create a payroll.PartTimeHourCalculator and call writePartTime with given data
    ...
    public static double calculateEarned(String department, String role, double hoursWorked) {
        //get list of salaries from employee.EmployeeSalary
    ...
    public static void writePartTime(String ppsNo, String hoursWorkedStr, String date) {
        ...
        //create a payroll.PartTimeHourCalculator and load employees via employee.EmployeeSalary
        PartTimeHourCalculator calculator = new PartTimeHourCalculator();

        //find employee by pps
        Employee employee = calculator.findEmployeeByPPSNo(ppsNo);
        if (employee != null) {
            System.out.println("employee.Employee Found: " + employee);
            ...
            if (!department.equalsIgnoreCase("ulac")) {
                System.out.println("employee.Employee is not part-time worker.");
                return;
            }
            ...
            // call writeCSVPartTime in csv.CSVWriter to write data to PartTime.csv
            CSVWriter.writeCSVPartTime(ppsNo, hoursWorked, date, earned);

        } else {
            System.out.println("employee.Employee with PPS No. " + ppsNo + " not found.");
        }
    }

    /**
     * Constructor to initialize the payroll.PartTimeHourCalculator and load employees from employee.EmployeeSalary.
     */
    public PartTimeHourCalculator() {
        employeeList = new ArrayList<>();
        loadEmployeesFromEmployeeSalary(); // load employees via employee.EmployeeSalary
    }

    /**
     * loads employee data from employee.EmployeeSalary and adds it to employeeList
     *
     *  @throws IOException if an error occurs while retrieving employee data from employee.EmployeeSalary
     */
    private void loadEmployeesFromEmployeeSalary() {
        try {
            // uses processEmployees() from employee.EmployeeSalary to get employee data
            List<Employee> employees = EmployeeSalary.processEmployees();
            employeeList.addAll(employees); //adds all employees to the list
        } catch (IOException e) {
            System.err.println("Error loading employees from employee.EmployeeSalary: " + e.getMessage());
        }
    }
